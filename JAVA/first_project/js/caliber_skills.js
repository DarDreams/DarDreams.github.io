//RoboTaskApp().AddUserVariable({TEXT},result)
//let text = "will_to_live";
let skills = {
    PERK_1: [
        "booster_pouch",
        "field_medicine",
...
function check (text) {
    for (let key in skills) {
       ...
       // skills[key].forEach(function (item, i, skills) {
       //console.log(skills[key][i]);
           if (skills[key][i] == text) {
                console.log(key,"-",i+1);
                //RoboTaskApp().AddUserVariable(key, i+1);
                return;
            }
        }
    }
